...
let pull connection =
  match%lwt (Database.Feeds.pull connection) with
    | Ok (Some feed) ->
      ...
      let items = document |> Option.map (fun (doc : Source.Rss.t) -> doc.items) |> Option.to_list |> List.flatten |> Common.Magic.List.take 10 in
      let _ = Dream.log "Puller.pull - storing %d items" (List.length items) in
      let%lwt _ = Common.Magic.Lwt.flatmap(fun item -> Database.Items.create item connection)(items) in
        Lwt.return ()
    | Ok None ->
      let _ = Dream.log "Puller.pull - nothing to pull" in
        Lwt.return ()
    | Error e ->
      let _  = Dream.log "Puller.pull - lookup failed with %s" (Api.Error.Database.to_string e) in
        Lwt.return ()
